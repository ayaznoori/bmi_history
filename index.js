...
app.patch('/userdata/:id',async(req,res)=>{
    ...
    if(check.length>0){
        let data1=check[0].bmi_history;
        data1.push((data.weight/((data.height*0.3048)**2)).toFixed(2));
        let update=await userModel.findByIdAndUpdate({_id:id},{bmi_history:data1})
